...
const char *parse_ident(void) {
    ...
    if (!strcmp(name, "=")) return 0;
    if (!strcmp(name, "let")) return 0;
    if (!strcmp(name, "axiom")) return 0;
    if (!strcmp(name, "in")) return 0;
...
int parse_let(void) {
    ...
    if (!param_name) {
        print_loc(pos);
        printf("Failed to parse the let binding, since its name is missing\n");
        longjmp(env, 1);
    }
    ...
    if (!parse_str("=")) {
        print_loc(pos);
        printf("Failed to parse the let binding, expected a '=' after the type\n");
        longjmp(env, 1);
    }
    ...
    if (!parse_str("in")) {
        print_loc(pos);
        printf("Failed to parse the end of the let binding, expected a 'in' as the final delimiter\n");
        longjmp(env, 1);
    }
    ...
    if (!input[pos]) {
        print_loc(pos);
        printf("Failed to parse program, missing final expression at the end of let bindings\n");
        longjmp(env, 1);
    }
...
int parse_axiom(void) {
    ...
    if (!parse_str("in")) {
        print_loc(pos);
        printf("Failed to parse the end of the axiom declaration, expected a 'in' as the final delimiter\n");
        longjmp(env, 1);
    }
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (!f) continue;
        char lbuf[16*2048];
        int len = fread(lbuf, 1, sizeof(lbuf), f);
        fclose(f);
        ...
        if (value == 0) {
            ...
            print_formatted_term(v);
            printf("\nwhich has type: ");
            print_formatted_term(t);
            printf("\n");
        }
    }
}
